...
RUN mkdir /build && \
    curl -fsSL "https://github.com/seerr-team/seerr/archive/${VERSION}.tar.gz" | tar xzf - -C "/build" --strip-components=1 && \
    cd /build && \
...
ARG VERSION
RUN curl -fsSL "https://github.com/seerr-team/seerr/archive/${VERSION}.tar.gz" | tar xzf - -C "${APP_DIR}" --strip-components=1 && \
